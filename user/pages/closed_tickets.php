...
<h3 class="block-title">Closed Tickets</h3>
</div>
...
$stmt = $conn->prepare("SELECT * FROM tbl_tickets LEFT JOIN tbl_ticket_categories ON tbl_tickets.category = tbl_ticket_categories.id WHERE tbl_tickets.member_id = ? AND tbl_tickets.status = '2' ORDER BY tbl_tickets.id DESC");
